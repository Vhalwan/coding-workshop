...
def create_entry(config, data):
    ...
    with conn.cursor() as cur:
        ...
        """, (data["project_id"], data.get("project_name"), data["category"], data.get("description"), data["amount"], data.get("type", "expense"), _optional_date(data.get("date")), data.get("created_by")))
        conn.commit()
...
def update_entry(config, entry_id, data):
    ...
    with conn.cursor() as cur:
        cur.execute("""
            ...
            WHERE id = %s
            RETURNING id, project_id, project_name, category, description, amount, type, date, created_by, created_at, updated_at;
        """, (data.get("category"), data.get("description"), data.get("amount"), data.get("type"), _optional_date(data.get("date")), entry_id))
        conn.commit()
        return row_to_dict(cur.fetchone())
